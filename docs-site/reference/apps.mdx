...
<CardGroup cols={2}>
  ...
  <Card title="Day-book" icon="book">
    Grouped by the merchant's own calendar day. Close of day shows what was sold and what became of it: received, still coming, needs a look, lost. The rebate choice, cash or SKR, is one tap away.
  </Card>
...
Pay without signal, and be paid by another customer the same way.

- **Once, online:** connect a wallet, make the payment key in the secure element, and open a vault with a first deposit.
- **Pay:** scan a till's code or another customer's receive code, confirm, and show the voucher.
- **Receive from someone:** show a receive code, scan the other phone's voucher, and accept it. It is checked offline with the till's own code, and settles through Nelo's relayer into your wallet on the next Sync.

Typechecks and bundles; never run on a handset. Settling what was received needs `EXPO_PUBLIC_NELO_RELAY_URL` and `EXPO_PUBLIC_NELO_RELAY_TOKEN` in `apps/payer/.env`, the same as the merchant app's.

Hosts `@nelo/attest`. The device checks below remain one tap away from the home screen.
...
The probe *detects and labels* this correctly, and the payer app says so on its welcome screen before a payer sets up.
</Warning>
