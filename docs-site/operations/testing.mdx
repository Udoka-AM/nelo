...
| `packages/queue` | 52 | `pnpm install` |
...
**502, all green, all on every push.** Everything but the LiteSVM suite runs with no phone and no validator — which is the whole reason the arithmetic was pulled out of the app and the program in the first place.
...
| Job | What | Cold | Warm |
|---|---|---|---|
| **TypeScript** | 431 tests, then four typechecks | ~21s | ~21s |
...
1. **Rebuild both development clients.** `expo-camera` is a new native module in both apps,
   ...
   install both APKs.
2. **Fund the merchant wallet with a little devnet SOL.** Redemptions are signed and paid for
   by the merchant's own wallet over Mobile Wallet Adapter.
...
5. **Both phones in airplane mode.** Till: enter an amount, **Charge**. Payer: **Pay a
   ...
   must be refused.
6. **Reconnect and tap the orange bar.** The wallet asks to sign; a wallet that adds a
   priority fee is refused on purpose, with a message saying so.
</Note>
