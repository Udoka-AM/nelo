...
| `packages/queue` | 51 | `pnpm install` |
| `packages/redeem` | 27 | `pnpm install` |
| `packages/accept` | 26 | Node only |
...
| `programs/nelo_vault` — golden vectors (voucher, redemption tx, error codes) | 5 | Rust only |
| `packages/reserve` | 17 | Node only |
| `packages/ledger` | 13 | Node only |
| `packages/attest` | 12 | Node only |

**431, all green, all on every push.** Everything but the LiteSVM suite runs with no phone and no validator — which is the whole reason the arithmetic was pulled out of the app and the program in the first place.
...
| Job | What | Cold | Warm |
|---|---|---|---|
| **TypeScript** | 362 tests, then four typechecks | ~21s | ~21s |
| **Rust (host)** | `cargo fmt`, clippy, 19 unit tests, 5 golden-vector tests | not yet timed | ~34s |
