...
| `packages/redeem` | 32 | `pnpm install` |
...
| `packages/issue` | 16 | `pnpm install` |
| `packages/ledger` | 13 | Node only |
| `packages/attest` | 12 | Node only |
| `packages/till` | 9 | `pnpm install` |
| `programs/nelo_vault` — golden vectors (voucher, redemption tx, error codes, account layouts) | 7 | Rust only |

**491, all green, all on every push.** Everything but the LiteSVM suite runs with no phone and no validator — which is the whole reason the arithmetic was pulled out of the app and the program in the first place.
...
| Job | What | Cold | Warm |
|---|---|---|---|
| **TypeScript** | 420 tests, then four typechecks | ~21s | ~21s |
...
1. **Rebuild the development client.** `expo-camera` is a new native module, so the dev build
   on the phone cannot load this JS: `pnpm --filter @nelo/merchant build:dev`, then install
   the new APK.
...
4. **Airplane mode, then Charge → "Customer has no signal? Scan their code."** Needs the
   payer's QR, and the payer app does not show one yet. That screen is the remaining piece of
   the week-3 gate.
5. **Reconnect and tap the orange bar.** The wallet asks to sign; a wallet that adds a
