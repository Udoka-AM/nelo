...
| `packages/issue` | ✅ Payer-side issuing, never signs one sequence twice (16 tests); ⚠️ no payer screen uses it yet |
...
- **The payer's screens.** Issuing (`packages/issue`) and the QR format (`NV1:` + base45) exist and are tested. The payer app does not yet enrol a vault, deposit, or show a voucher as a QR, so the week-3 gate needs that screen before it can run.
