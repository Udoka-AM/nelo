...
<CardGroup cols={2}>
  ...
  <Card title="The ordinary sale" icon="receipt">
    **Written.** Till, MWA wallet, Solana Pay, payment validation, day-book, balance, SPL collateral.

    Caveat: never bundled. See [Testing](/operations/testing).
  </Card>
...
| `packages/onboard` | ✅ Proven (31 tests) |
...
| `packages/onboard` — phone + payout validation | ✅ Proven (31 tests) |
| Onboarding (embedded wallet) | ❌ Privy wiring not started |
...
<Steps>
  <Step title="Get a build onto a handset">
    Everything app-side is unverified until this happens, and `@nelo/attest`'s Kotlin has never been compiled at all. A first EAS build surfaces an entire class of bug in its first thirty seconds — one of which [has already been found by hand](/operations/testing#what-that-gap-already-cost).
  </Step>
