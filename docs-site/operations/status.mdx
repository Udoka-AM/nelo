...
| `services/relay` | 🟡 Policy + Kora client proven (25 tests); builder is week 3 |
...
- **Settling for Privy merchants.** Redemptions are signed by the merchant's wallet over Mobile Wallet Adapter. Privy's embedded signer takes `@solana/web3.js` objects this app does not carry, so a Privy merchant's offline payments stay queued until that is wired.
