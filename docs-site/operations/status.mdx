...
| `packages/queue` | ✅ Proven (51 tests) against a fake network; ⚠️ not wired into the merchant app |
...
- **The offline transport.** QR and NFC, and cached enrolment and revocation lists, are week-3 work. The offline queue exists (`packages/queue`) but no app feeds it yet, because nothing reads a voucher off a payer's screen.
