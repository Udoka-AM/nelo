...
| `packages/queue` | ✅ Proven (52 tests) against a fake network; wired into the merchant app, never run on a handset |
...
| `packages/redeem` | ✅ Matches the program's own builders byte for byte (27 tests); ⚠️ never broadcast to a cluster |
...
| `services/relay` | ✅ Submits offline vouchers and pays fees, idempotently (43 tests); runs on the Mac behind a tunnel for the demo |
...
| Slashing | ✅ Stake moves to the reserve (5 LiteSVM tests); ❌ nothing pays out of it yet |
